...
const MESSAGES: Record<string, string> = {
  ...
  UNSUPPORTED_MEDIA_TYPE:
    'This type of file is not allowed, or its contents did not match its type.',
  MALWARE_DETECTED: 'The malware scanner rejected this file.',
  SCAN_FAILED:
    'The malware scanner is unavailable, so this file was held and cannot be shared yet.',
...
export function precheck(file: File): UploadError | null {
  if (file.size > UPLOAD.MAX_BYTES)
    return new UploadError('PAYLOAD_TOO_LARGE', MESSAGES.PAYLOAD_TOO_LARGE ?? '');
  ...
  try {
    ...
    if (axios.isAxiosError<ApiErrorBody>(err)) {
      ...
      throw new UploadError(
        code,
        MESSAGES[code] ?? err.response?.data?.error?.message ?? 'Upload failed.',
      );
    }
...
export async function downloadFile(fileId: string): Promise<void> {
  ...
  const origin = new URL(api.defaults.baseURL ?? window.location.origin, window.location.origin)
    .origin;
