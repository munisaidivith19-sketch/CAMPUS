...
| **Push notifications** | ⬜ Optional | Mobile push (Phase 3+) | Expo Push (free) via Expo project | *(Expo project id in app config)* | Expo Push service — free; in-app + email cover the rest |
| **File storage** | ⬜ Optional | Uploaded files (Phase 3) | Any S3-compatible store for prod | `STORAGE_DRIVER`, `CLOUD_STORAGE_KEY`, `CLOUD_STORAGE_SECRET` | **Local disk** (`STORAGE_DRIVER=local`) — default, ₹0 |
| **ClamAV** | ⬜ Optional | Upload malware scanning (Phase 3/4) | Bundled Docker image | `CLAMAV_HOST`, `CLAMAV_PORT`, `CLAMAV_ENABLED` | `clamav/clamav` Docker service (`--profile scan`) — free |
